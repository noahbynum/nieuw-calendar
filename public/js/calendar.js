(function () {
  ...
  function isPastEvent(event, now) {
    now = now || new Date();
    return startOfDay(addDays(eventEndDay(event), 1)) < startOfDay(now);
  }
  ...
  function formatTime(time, timeFormat) {
    ...
    if (String(timeFormat) === "24") return pad(h) + ":" + pad(m);
  ...
  function formatEventWhen(event, settings) {
    ...
    if (event.allDay) return multi ? startLabel + " – " + endLabel : startLabel + " · " + t("allDay", "All day");
    ...
    if (multi) return (startLabel + " " + startT + " – " + endLabel + (endT ? " " + endT : "")).trim();
  ...
  function copyTheme(fromEl, toEl) {
    ...
    ["primary","secondary","text","bg","header","header-text","border","button","button-text","radius","font","font-heading","surface","muted","popup","popup-text","popup-muted"].forEach(function (k) {
  ...
  function coverHtml(event, extraClass, settings) {
    ...
    if (event.image) {
      return '<div class="' + cls + '" style="background-image:url(\'' + esc(event.image) + "')\" aria-hidden=\"true\"></div>";
    }
    var color = eventColor(event, settings);
    return '<div class="' + cls + '" style="background:linear-gradient(160deg,' + esc(color) + "," + darken(color, 0.42) + ')" aria-hidden="true">' + COVER_SVG + "</div>";
  }

  function catPills(event) {
    return (event.categories || []).map(function (c) {
      var col = c.color || "#2f5d50";
      return '<span class="nc-cat" style="background:' + esc(col) + ";color:" + contrastText(col) + '">' + esc(c.name) + "</span>";
    }).join("");
  }
  ...
  function mount(root) {
    ...
    function eventsVisible() {
      return (DATA.events || []).filter(function (e) {
        if (cat !== "all") {
          var ids = (e.categories || []).map(function (c) { return String(c.id); });
    ...
    function render() {
      ...
      var html = '<div class="nc-main"><div class="nc-toolbar">';
      html += '<div class="nc-month-nav"><button type="button" class="nc-btn nc-btn-ghost nc-btn-icon" data-act="prev" aria-label="' + esc(t("prev", "Previous month")) + '">' + ICONS.chevronLeft + "</button>";
      html += '<h2 class="nc-month-label">' + esc(monthTitle(cursor)) + "</h2>";
      html += '<button type="button" class="nc-btn nc-btn-ghost nc-btn-icon" data-act="next" aria-label="' + esc(t("next", "Next month")) + '">' + ICONS.chevronRight + "</button></div>";
      html += '<div class="nc-toggle" role="tablist"><button type="button" class="nc-toggle-btn' + (view === "month" ? " is-on" : "") + '" data-act="month" role="tab" aria-selected="' + (view === "month" ? "true" : "false") + '">' + ICONS.calendar + esc(t("month", "Month")) + "</button>";
      html += '<button type="button" class="nc-toggle-btn' + (view === "list" ? " is-on" : "") + '" data-act="list" role="tab" aria-selected="' + (view === "list" ? "true" : "false") + '">' + ICONS.list + esc(t("list", "List")) + "</button></div></div>";
      html += '<div class="nc-filters"><button type="button" class="nc-pill' + (cat === "all" ? " is-on" : "") + '" data-cat="all">' + esc(t("all", "All")) + "</button>";
      (DATA.categories || []).forEach(function (c) {
        html += '<button type="button" class="nc-pill' + (String(cat) === String(c.id) ? " is-on" : "") + '" data-cat="' + esc(c.id) + '">' + (c.color ? '<span class="nc-dot" style="background:' + esc(c.color) + '" aria-hidden="true"></span>' : "") + esc(c.name) + "</button>";
      });
      html += "</div>";
      html += view === "month" ? monthHtml(visible) : listHtml(listEvents(visible));
      var tz = String(settings.timezone || "").replace(/_/g, " ");
      if (tz) html += '<p class="nc-tz">' + esc(t("times", "Times shown in")) + " " + esc(tz) + ".</p>";
      html += "</div>";
      root.innerHTML = html;
    ...
    function monthHtml(visible) {
      ...
      labels.forEach(function (d) { html += "<span>" + esc(d) + "</span>"; });
      ...
      weeks.forEach(function (week) {
        ...
        week.forEach(function (day) {
          ...
          html += '<div class="nc-day' + (inMonth ? "" : " is-out") + (isToday ? " is-today" : "") + '"><span class="nc-day-num">' + day.getDate() + "</span></div>";
        });
        html += '<div class="nc-lanes">';
        lanes.slice(0, 3).forEach(function (lane) {
          ...
          lane.forEach(function (event) {
            ...
            var i;
            for (i = 0; i < 7; i++) {
              if (sameDay(week[i], s) || week[i] >= s) { startIdx = i; break; }
              startIdx = 6;
            }
            var endIdxRaw = -1;
            for (i = 0; i < 7; i++) {
              if (sameDay(week[i], e)) { endIdxRaw = i; break; }
            }
            var endIdx = endIdxRaw === -1 ? (e < week[0] ? 0 : 6) : endIdxRaw;
            var left = (startIdx / 7) * 100;
            var width = ((endIdx - startIdx + 1) / 7) * 100;
            var color = eventColor(event, settings);
            html += '<button type="button" class="nc-chip" data-event="' + esc(event.id) + '" title="' + esc(event.title) + '" style="left:calc(' + left + "% + 2px);width:calc(" + width + "% - 4px);background:" + esc(color) + ";color:" + contrastText(color) + ";border-radius:" + chipRadius + 'px">' + esc(event.title) + "</button>";
          });
          html += "</div>";
        });
        if (lanes.length > 3) html += '<div class="nc-more">+' + (lanes.length - 3) + " " + esc(t("more", "more")) + "</div>";
        html += "</div></div>";
      });
      html += "</div></div>";
      return html;
    ...
    function listHtml(items) {
      if (!items.length) {
        return '<div class="nc-list-empty"><p class="nc-list-empty-title">' + esc(t("emptyTitle", "No events in this view")) + '</p><p class="nc-list-empty-body">' + esc(t("emptyBody", "Try another category, or add an event from the dashboard.")) + "</p></div>";
      }
      var groups = [];
      ...
      groups.forEach(function (group) {
        html += '<section><h3 class="nc-list-heading">' + esc(listHeading(group.day)) + '</h3><ul class="nc-list-items">';
        group.items.forEach(function (event) {
          var color = eventColor(event, settings);
          html += '<li><button type="button" class="nc-card" data-event="' + esc(event.id) + '">' + coverHtml(event, "nc-cover-list", settings) + '<span class="nc-stripe" style="background:' + esc(color) + '"></span><span class="nc-card-body"><span class="nc-card-row"><span class="nc-card-title">' + esc(event.title) + "</span>" + catPills(event) + '</span><span class="nc-when">' + esc(formatEventWhen(event, settings)) + "</span>" + (event.location ? '<span class="nc-loc">' + ICONS.pin + "<span>" + esc(truncate(event.location, 120)) + "</span></span>" : "") + "</span></button></li>";
        });
        html += "</ul></section>";
      });
      return html + "</div>";
    ...
    function openModal() {
      ...
      if (event.colorOverride) {
        cats += '<span class="nc-cat" style="background:' + esc(color) + ";color:" + contrastText(color) + '">' + esc(t("custom", "Custom color")) + "</span>";
      }
      dialog.innerHTML =
        coverHtml(event, "nc-cover-dialog", settings) +
        '<div class="nc-dialog-pane"><div class="nc-dialog-scroll"><div class="nc-dialog-body"><div class="nc-dialog-top"><h2 class="nc-dialog-title" id="nc-dialog-title">' +
        esc(event.title) +
        '</h2><button type="button" class="nc-dialog-close" aria-label="' +
        esc(t("close", "Close")) +
        '">' + ICONS.x + "</button></div><p class=\"nc-dialog-when\">" +
        esc(formatEventWhen(event, settings)) +
        "</p>" +
        (event.location ? '<p class="nc-dialog-loc">' + ICONS.pin + "<span>" + esc(truncate(event.location, 180)) + "</span></p>" : "") +
        (cats ? '<div class="nc-dialog-cats">' + cats + "</div>" : "") +
        (event.description ? '<p class="nc-dialog-desc">' + esc(event.description) + "</p>" : "") +
        '</div></div><div class="nc-scrollbar" aria-hidden="true"><div class="nc-scrollbar-thumb"></div></div></div>';
